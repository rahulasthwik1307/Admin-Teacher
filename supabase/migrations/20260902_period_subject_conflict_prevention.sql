-- Migration: AttendGuard Period/Subject Conflict Prevention
-- Guarantees:
-- 1. SAME teacher + class + subject + period + date = SAME LOGICAL SESSION (reopen/resume existing session, 0 duplicate rows)
-- 2. SAME teacher + class + period + date + DIFFERENT subject = SLOT CONFLICT (blocked, 0 rows created, existing session untouched)
-- 3. Atomic advisory locking on slot identity (teacher_id + class_id + period_id + session_date) to serialize concurrent requests
...
-- 2. Atomic Stored Procedure: Start, Resume, Reopen, or Conflict QR Attendance Session
...
BEGIN
  ...
  -- This ensures all concurrent requests for the same slot (same or different subject) are serialized atomically
  ...
  -- 5. RULE 1 & 6 — Check if session already exists for this exact logical slot today (SAME subject)
  ...
  -- 6. RULE 2 & 7 — Check if another DIFFERENT subject already occupies this teacher/class/period/date slot
