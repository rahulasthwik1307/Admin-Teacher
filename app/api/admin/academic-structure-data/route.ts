...
export async function GET() {
  try {
    ...
    const [deptsRes, classesRes, subjectsRes, periodsRes, classCountRes, subjectCountRes] = await Promise.all([
      ...
      supabase.from("classes").select("id, name, section, year, department:departments ( name, code )").order("name"),
    ...
    return NextResponse.json({
      ...
      classes: (classesRes.data ?? []).map((c: any) => ({ id: c.id, name: c.name, section: c.section, year: c.year, department: c.department?.code ?? "—", departmentFull: c.department?.name ?? "—", displayName: `${c.name}-${c.section} · ${c.year}` })),
